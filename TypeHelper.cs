    ...
    public static class TypeHelper
    {
        ...
        private sealed class TypeKey
        {
                ...
                obj)
            {
                ...
                {
                    int localHash = GetHashCode();
                    int otherHash = other.GetHashCode();
                    if (localHash == otherHash)
                    {
                        if (other.AsmName == null && AsmName == null)
                        {
                            result = true;
                        }
                        else if (other.AsmName != null && AsmName != null)
                        {
#if NET40 || NETSTANDARD2_0
                            if (other.AsmName.KeyPair != null && AsmName.KeyPair != null)
                            {
                                result = Arrays.DeepEquals(other.AsmName.KeyPair.PublicKey, AsmName.KeyPair.PublicKey);
                            }
                            else if (other.AsmName.KeyPair == null && AsmName.KeyPair == null)
                            {
                                result = true;
                            }
#else
                            string pkt = string.Empty;
                            string pktValueOther = string.Empty;
                            int index = other.AsmName.FullName.IndexOf("PublicKeyToken=");
                            if (index != -1)
                            {
                                pkt = other.AsmName.FullName.Substring(index);
                                pktValueOther = pkt.Split(new string[] { "=" }, StringSplitOptions.RemoveEmptyEntries)[1];
                            }

                            pkt = string.Empty;
                            string pktValueThis = string.Empty;
                            index = AsmName.FullName.IndexOf("PublicKeyToken=");
                            if (index != -1)
                            {
                                pkt = AsmName.FullName.Substring(AsmName.FullName.IndexOf("PublicKeyToken="));
                                pktValueThis = pkt.Split(new string[] { "=" }, StringSplitOptions.RemoveEmptyEntries)[1];
                            }
